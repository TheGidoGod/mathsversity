(function () {
  ...
  function render() {
    ...
    roundSolved = false;
    document.getElementById("checkTool").onclick = () => { const selected = document.querySelector(".choice.selected")?.dataset.answer; const feedback = document.getElementById("feedback"); if (selected === answer && !roundSolved) { roundSolved = true; score += 1; streak += 1; document.getElementById("score").textContent = score; document.getElementById("scoreMeta").textContent = `Streak ${streak} · Tries ${tries}`; document.getElementById("checkTool").disabled = true; document.getElementById("checkTool").textContent = "Solved!"; window.mathsversityFreemium?.consumeHeart(); window.mathsversityCelebrate?.(); feedback.textContent = `Correct — ${answer}.`; feedback.className = "feedback good"; } else if (!roundSolved) { tries += 1; streak = 0; document.getElementById("scoreMeta").textContent = `Streak ${streak} · Tries ${tries}`; feedback.textContent = selected ? "Not quite. Use the visual model and try again." : "Choose an answer first."; feedback.className = "feedback try"; } };
