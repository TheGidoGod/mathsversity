(function () {
  ...
  function render() {
    ...
    document.getElementById("checkTool").onclick = () => { const selected = document.querySelector(".choice.selected")?.dataset.answer; const feedback = document.getElementById("feedback"); if (selected === answer) { score += 1; document.getElementById("score").textContent = score; window.mathsversityFreemium?.consumeHeart(); window.mathsversityCelebrate?.(); feedback.textContent = `Correct — ${answer}.`; feedback.className = "feedback good"; } else { feedback.textContent = selected ? "Not quite. Use the visual model and try again." : "Choose an answer first."; feedback.className = "feedback try"; } };
